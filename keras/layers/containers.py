...
from ..layers.MyLayers import PairMerge, TripletMerge
...
class Graph(Layer):
    ...
    def add_triplet_merge_node(self, name, input1, input2, input3,
                 merge_mode='concat', concat_axis=-1, dot_axes=-1,
                 create_output=False):
        '''Add a node in the graph. It can be connected to multiple
        inputs, which will first be merged into one tensor
        ...
        merge = TripletMerge(to_merge[0], to_merge[1], to_merge[2], mode=merge_mode) 

        self.namespace.add(name)
        self.nodes[name] = merge
        self.node_config.append({'name': name,
                                 'input1': input1,
                                 'input2': input2,
                                 'input3': input3,
