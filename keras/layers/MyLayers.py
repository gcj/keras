...
class PairMerge(Layer):
    ...
    """
    def __init__(self, input1, input2, include_previous_layers=False, mode='sum', concat_axis=-1, dot_axes=-1):
        ...
        self.constraints = []
        self.updates = []
        if include_previous_layers == True:
            #print 'Yeah'
...
class TripletMerge(Layer):
    """
    merge a pair of inputs
    """
    def __init__(self, input1, input2, input3, include_previous_layers=False, mode='sum', concat_axis=-1, dot_axes=-1):
        if mode not in {'sum', 'mul', 'concat', 'ave', 'join', 'cos', 'dot',
                        'l2', 'l1', 'hamming', 'l2diff_relu'}:
            raise Exception('Invalid merge mode: ' + str(mode))
        
        self.mode = mode
        self.concat_axis = concat_axis
        self.dot_axes = dot_axes
        self.layers = [input1, input2, input3]
        self.trainable_weights = []
        self.regularizers = []
        self.constraints = []
        self.updates = []
        if include_previous_layers == True:
            ...
            for l in self.layers:
                ...
                #print self.trainable_weights
            #print 'OOH'
        super(TripletMerge, self).__init__()
    ...
    def get_output(self, train=False):
        s1 = self.layers[0].get_output(train)
        s2 = self.layers[1].get_output(train)
        s3 = self.layers[2].get_output(train)
        if self.mode == 'sum':
            return s1 + s2 + s3
        elif self.mode == 'ave':
            return (s1 + s2 + s3)/3
            ...
            return s1 * s2 * s3
        elif self.mode == 'l2diff_relu':
            return sigmoid(K.sum(K.square(s1-s2), axis=-1, keepdims=True) - K.sum(K.square(s1-s3), axis=-1, keepdims=True)) 
        elif self.mode == 'l1':
            return K.sum(K.abs(s1-s2))
        else:
            raise Exception('Unknown merge mode.')
